...
pData(dat.relative) %>%
  ...
  geom_vline(xintercept = read_min, lty="dashed") +
...
table(pData(dat.relative[,remove])$age, pData(dat.relative[,remove])$sex,pData(dat.relative[,remove])$genotype)
...
dat.relative.pca<-prcomp(t(exprs(dat.relative.filtered)),scale=F,center=F)
# If scale=T:
# Error in prcomp.default(t(exprs(dat.relative)), scale = T, center = T) : 
...
p7<-ggbiplot(dat.relative.filtered.pca,var.axes=F,groups=(dat.relative.filtered.pca$x[,1] > 1e+06)) +
  theme(aspect.ratio = 1, legend.position="none") + 
  scale_color_manual(values=c("black", "red")) +
...
pg1<-ggplot(pData(dat.relative)) +
  ...
  scale_color_manual(values=c("black","red")) +
  theme(legend.position = c(0.9,0.94))
...
save_plot("removed_cells.pdf", pg, base_height=16, base_width = 22)
```
