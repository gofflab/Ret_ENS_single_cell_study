custom_show.velocity.on.embedding.cor <-
  function (emb, vel, n = 100, cell.colors = NULL, corr.sigma = 0.05, 
          ...
          return.details = FALSE, expression.scaling = FALSE, ...) 
{
  ...
  if (!is.null(show.cell)) {
    ...
    velocity <- data.frame(t(velocyto.R:::embArrows(emb, tp, arrowScale = 1, 
                                                    n.cores)))
    rownames(velocity) <- rownames(emb)   
    arsd <- velocity * arrow.scale
    colnames(velocity) <- c("Vx", "Vy")
  ...
  return(invisible(list(tp = tp, cc = cc, velocity = velocity)))
}
