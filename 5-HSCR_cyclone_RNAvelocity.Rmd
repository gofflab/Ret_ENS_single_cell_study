...
p1 <- ggplot(pData(dat.filtered), aes(x = UMAP1, y = UMAP2, color = G1)) +
  geom_point(size = 0.1) +
  ggtitle("G1") +
  scale_color_viridis("Probability", limits = c(0, 1), breaks = c(0, 1), option = "inferno") +
...
pdf("Cyclone_probabilities.pdf", width = 6, height = 5.5, useDingbats = F)
grid.arrange(p1, p2, p3, ncol = 3)
dev.off()

#subset projection weights by condition (age*genotype)
a <- pData(dat.filtered)[pData(dat.filtered)$age == "E12.5" & pData(dat.filtered)$genotype == "het", c("G1", "S", "G2M")]
b <- pData(dat.filtered)[pData(dat.filtered)$age == "E12.5" & pData(dat.filtered)$genotype == "hom", c("G1", "S", "G2M")]
c <- pData(dat.filtered)[pData(dat.filtered)$age == "E14.5" & pData(dat.filtered)$genotype == "het", c("G1", "S", "G2M")]
d <- pData(dat.filtered)[pData(dat.filtered)$age == "E14.5" & pData(dat.filtered)$genotype == "hom", c("G1", "S", "G2M")]

# 6 pairwise tests for each pattern
# Bonferonni correction - multiply p by 6
# * p < 0.05/6
# ** p < 0.01/6

#G1
ks.test(a$G1, b$G1) # p = 0.283 (E12.5 genotype) ### n.s.
ks.test(a$G1, c$G1) # p = 1.72e-6 (het age) ######## **
ks.test(a$G1, d$G1) # p = 1.80e-5 (age*genotype) ### **
ks.test(b$G1, c$G1) # p = 4.75e-8 (age*genotype) ### **
ks.test(b$G1, d$G1) # p = 6.59e-6 (hom age) ######## **
ks.test(c$G1, d$G1) # p = 2.14e-3 (E14.5 genotype) # *

#S Phase
ks.test(a$S, b$S) # p = 0.763 (E12.5 genotype) # n.s.
ks.test(a$S, c$S) # p < 2.2e-16 (het age) ###### **
ks.test(a$S, d$S) # p = 1.35e-4 (age*genotype) # **
ks.test(b$S, c$S) # p < 2.2e-16 (age*genotype) # **
ks.test(b$S, d$S) # p = 1.94e-4 (hom age) ###### **
ks.test(c$S, d$S) # p = 0.634 (E14.5 genotype) # n.s.

#G2M Phase
ks.test(a$G2M, b$G2M) # p = 0.245 (E12.5 genotype) ## n.s.
ks.test(a$G2M, c$G2M) # p = 9.34e-4 (het age) ####### **
ks.test(a$G2M, d$G2M) # p = 0.0184 (age*genotype) ### n.s.
ks.test(b$G2M, c$G2M) # p = 6.88e-4 (age*genotype) ## **
ks.test(b$G2M, d$G2M) # p = 1.55e-3 (hom age) ####### **
ks.test(c$G2M, d$G2M) # p = 0.0955 (E14.5 genotype) # n.s.
...
pData(dat.filtered)$cyclone_class <- c("G1", "S", "G2M")[apply(pData(dat.filtered)[,c("G1", "S", "G2M")], 1, which.max)]
pData(dat.filtered)$cyclone_class <- factor(pData(dat.filtered)$cyclone_class, levels = c("G1", "S", "G2M"))
pData(dat.filtered)$cyclone_maxP <- apply(pData(dat.filtered)[,c("G1", "S", "G2M")], 1, max)


p <- ggplot(pData(dat.filtered)) +
  geom_point(aes(x=UMAP1, y=UMAP2, 
                 color = cyclone_class,
                 alpha = cyclone_maxP), size = 0.7,
             shape = 16) +
  scale_color_manual(values = brewer.pal(n = 7, name = "Dark2")[c(7, 1, 2)]) +
...
pdf("cyclone_MPM.pdf", width = 3, height = 3, useDingbats = F)
p
dev.off()
...
p1 <- pData(dat.filtered) %>%
  ...
  scale_x_continuous("Probability", expand = c(0, 0), breaks = c(0, 0.5, 1), labels = c("0", "0.5", "1"), limits = c(0, 1.001)) +
  scale_color_manual("Age, Genotype", values = c("E12.5 het" = "#E41A1C", "E12.5 hom" = "#377EB8", "E14.5 het" = "#E41A1C", "E14.5 hom" = "#377EB8")) +
  scale_linetype_manual("Age, Genotype", values = c("E12.5 het" = "solid", "E12.5 hom" = "solid", "E14.5 het" = "dotted", "E14.5 hom" = "dotted")) +
  theme(aspect.ratio = 1, 
...
pdf("cyclone_ecdf_MPM.pdf", width = 10.5, height = 2, useDingbats = F)
...
p4 <- CC_patterns$projection %>%
  mutate(condition = paste(age, genotype)) %>%
  ggplot(., aes(max_value, color = condition, lty = condition)) + 
  stat_ecdf(geom = "step") +
  ggtitle("Max Value") +
  scale_y_continuous("Cumulative Density", expand = c(0, 0), breaks = c(0, 0.5, 1), labels = c("0", "0.5", "1")) +
  scale_x_continuous("Pattern Weight", expand = c(0, 0), breaks = c(0, 0.5, 1), labels = c("0", "0.5", "1"), limits = c(0, 1.001)) +
...
pdf("CC_pattern_ecdf_MPM.pdf", width = 10.5, height = 3, useDingbats = F)
grid.arrange(p1, p2, p3, p4, nrow = 1)
dev.off()

#subset projection weights by condition (age*genotype)
a <- CC_patterns$projection[CC_patterns$projection$age == "E12.5" & CC_patterns$projection$genotype == "het", c(paste0("Pattern_", 40:42), "max_value")]
b <- CC_patterns$projection[CC_patterns$projection$age == "E12.5" & CC_patterns$projection$genotype == "hom", c(paste0("Pattern_", 40:42), "max_value")]
c <- CC_patterns$projection[CC_patterns$projection$age == "E14.5" & CC_patterns$projection$genotype == "het", c(paste0("Pattern_", 40:42), "max_value")]
d <- CC_patterns$projection[CC_patterns$projection$age == "E14.5" & CC_patterns$projection$genotype == "hom", c(paste0("Pattern_", 40:42), "max_value")]
...
ks.test(a$Pattern_40, c$Pattern_40) # p < 2.2e-16 **, E12.5 het vs. E14.5 het (age)
  ks.test(a$Pattern_40, c$Pattern_40)[[2]]*6
...
ks.test(b$Pattern_42, d$Pattern_42) # p = 3.234e-10 ** (hom age)
  ks.test(b$Pattern_42, d$Pattern_42)[[2]]*6
ks.test(c$Pattern_42, d$Pattern_42) # p = 0.003222 * (E14.5 genotype)
  ks.test(c$Pattern_42, d$Pattern_42)[[2]]*6

ks.test(a$max_value, b$max_value) # p = 0.1784 (E12.5 genotype)
ks.test(a$max_value, c$max_value) # p < 2.2e-16 ** (het age)
ks.test(a$max_value, d$max_value) # p < 2.2e-16 ** (age*genotype)
ks.test(b$max_value, c$max_value) # p < 2.2e-16 ** (age*genotype)
ks.test(b$max_value, d$max_value) # p = 2.2e-16 ** (hom age)
ks.test(c$max_value, d$max_value) # p = 1.907e-5 ** (E14.5 genotype)
