...
```{r CC_patterns}
CC_patterns <- list()
CC_patterns$projection <- t(projection70$projection[paste0("Pattern_", 40:42),]) #Subset CC patterns
CC_patterns$pval <- t(projection70$pval[paste0("Pattern_", 40:42),]) #Subset CC patterns


CC_patterns$projection <- apply(CC_patterns$projection, 2, function(x){ #Normalize to [0,1]
  x=x-min(x)
  x=x/max(x)
})

CC_patterns$projection <- merge(pData(dat.filtered)[,c("cell_id", "UMAP1", "UMAP2", "age", "sex", "genotype")],
...
melted_CC_patterns <- merge(melted_CC_patterns_projection, melted_CC_patterns_pval, by.x = c("cell_id", "variable"), by.y = c("Var1", "Var2"))
colnames(melted_CC_patterns) <- c("cell_id", "pattern", "x", "y", "age", "sex", "genotype", "weight", "pval")

phase <- c("Pattern_40" = "S", 
       "Pattern_41" = "Early M", 
       "Pattern_42" = "Late M")



tmp <- merge(linnarsson_dat[,c("X", "Y","parentCellType")], linnarsson_gapsresult@sampleFactors[,40:42], by = 0)
tmp <- melt(tmp, id.vars = c("Row.names", "X", "Y", "parentCellType"))
names(tmp) <- c("cell_id", "x", "y", "subset", "pattern", "weight")
tmp$significant <- TRUE

melted_CC_patterns$significant <- melted_CC_patterns$pval < 0.05/nrow(pData(dat.filtered))
melted_CC_patterns$subset <- "HD"
tmp2 <- rbind(tmp, melted_CC_patterns[,names(tmp)])
tmp2$subset <- factor(tmp2$subset, levels = c("glia", "neuron", "HD"))
...
p2 <- ggplot(CC_patterns$projection) +
...
p1 <- ggplot(tmp2[tmp2$pattern == "Pattern_40",]) +
  ...
  ggtitle("Pattern 40\nS Phase") +
  facet_wrap(~subset, scales = "free", ncol = 3) +
#  scale_color_gradient("Pattern weight", low = "#F1F1F1", high = "#1B9E77", guide = FALSE) +
...
pdf("CC_pattern_projections_v2.pdf", width = 6, height = 5.5, useDingbats = F)
grid.arrange(p1, p2, p3, ncol = 1)
dev.off()
...
# ** p < 0.05/6
# *** p < 0.01/6
ks.test(a$Pattern_40, b$Pattern_40) # p = 0.1931
ks.test(a$Pattern_40, c$Pattern_40) # p < 2.2e-16 **
ks.test(a$Pattern_40, d$Pattern_40) # p < 2.2e-16 **
ks.test(b$Pattern_40, c$Pattern_40) # p < 2.2e-16 **
ks.test(b$Pattern_40, d$Pattern_40) # p = 3.331e-16 **
ks.test(c$Pattern_40, d$Pattern_40) # p = 1.609e-5 **

ks.test(a$Pattern_41, b$Pattern_41) # p = 0.02552 
ks.test(a$Pattern_41, c$Pattern_41) # p < 2.2e-16 **
ks.test(a$Pattern_41, d$Pattern_41) # p < 2.2e-16 **
ks.test(b$Pattern_41, c$Pattern_41) # p < 2.2e-16 **
ks.test(b$Pattern_41, d$Pattern_41) # p = 6.395e-14 **
ks.test(c$Pattern_41, d$Pattern_41) # p = 3.279e-5 **

ks.test(a$Pattern_42, b$Pattern_42) # p = 0.3977
ks.test(a$Pattern_42, c$Pattern_42) # p < 2.308e-9 **
ks.test(a$Pattern_42, d$Pattern_42) # p < 2.976e-9 **
ks.test(b$Pattern_42, c$Pattern_42) # p < 1.591e-11 **
ks.test(b$Pattern_42, d$Pattern_42) # p = 3.234e-10 **
ks.test(c$Pattern_42, d$Pattern_42) # p = 0.003222 *
...
show.velocity.on.embedding.cor(as.matrix(emb), vel, n = 50, scale = 'sqrt', cell.colors = ac(cell.colors, alpha = cell.alpha), cex = cell.cex, arrow.scale = 30, show.grid.flow = TRUE, min.grid.cell.mass = 0.5, grid.n = 30, arrow.lwd = 2, cell.border.alpha = 0)

velocities <- merge(p$velocity, pData(dat.filtered)[, c("age", "sex", "genotype", "cluster", "final_celltype", "CC_phase")], by = 0)
