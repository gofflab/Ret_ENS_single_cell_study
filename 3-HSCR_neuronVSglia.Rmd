...
dat.neurons <- dat.filtered[,pData(dat.filtered)$CellType %in% c("Neuron","Noradrenergic Neuron","Acetylcholinergic Neuron","Transition Cell") & !pData(dat.filtered)$cluster == 8]
# 385 cells
...
glia_genotype_cluster_diff_test <- differentialGeneTest(dat.glia[glia_expressed_genes, pData(dat.glia)$cluster %in% c(1, 2, 5, 6, 8, 10)], fullModelFormulaStr = "~genotype*cluster + sex + batch + CellType", reducedModelFormulaStr = "~genotype + sex + cluster + batch + CellType", verbose = T)
