...
```{r}
tmp <- dat.filtered[,pData(dat.filtered)$cluster %in% c(4, 9)]

neuron_bifuracation_difftest <- differentialGeneTest(tmp[expressed_genes,], fullModelFormulaStr = "~batch + cluster", reducedModelFormulaStr = "~batch", cores = detectCores() - 1)
...
# het-only = 10, mixed pop = 9

pop4_means <- apply(exprs(tmp)[neuron_bifuracation_difftest_sorted_sig_genes,pData(tmp)$cluster == 4], 1,mean)
pop9_means <- apply(exprs(tmp)[neuron_bifuracation_difftest_sorted_sig_genes,pData(tmp)$cluster == 9], 1,mean)
