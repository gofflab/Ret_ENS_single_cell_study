...
projection_embedding <- umap(t(projection70$projection), random_state = 100L)
...
```{r}
tmp <- t(projection70$projection*(1-projection70$pval)) # scale by 1-p value

projection_embedding <- umap(tmp, random_state = 100L)
...
tmp <- t(projection70$projection)
tmp <- apply(tmp, 2, function(x){ (x - mean(x)) / sd(x) }) # scale to mean 0 sd 1
...
tmp <- tmp*t(1-projection70$pval) # then scale by p value
...
```{r projection_normalized_unsup_clustering}
mat <- exprs(dat.filtered.expressed.normalized[unsup_clustering_expressed_genes,])
rownames(mat) <- gsub("\\..*", "", rownames(mat))
mat <- log10(mat+1)

projection70_normalized <- projectR(mat, linnarsson70, AnnotionObj = NA, IDcol=NA,full=TRUE)

tmp <- projection70_normalized$projection
rownames(tmp) <- paste0("Linnarsson_", rownames(tmp))
...
pdf("UMAP_projections_normalized_np70.pdf")
for (i in seq_along(plot_list)) {
    print(plot_list[[i]])
}
dev.off()
...
plot_list = list()
for (i in 1:70) {
    pval <- projection70_log$pval[i,]
    value <- projection70_log$projection[i,]
    p = plotUMAP(tmp, color_by = ifelse(pval < 0.05, value, NA), alpha = ifelse(pval < 0.05, 1, 0.1)) + 
      ggtitle(paste("Linnarsson Pattern", i)) +
      scale_color_viridis(option = "C")
    plot_list[[i]] = p
}

pdf("UMAP_projections_log10_np70_significant.pdf")
for (i in seq_along(plot_list)) {
    print(plot_list[[i]])
}
dev.off()

projection_embedding <- umap(t(projection70_normalized$projection), random_state = 100L)
...
projection_embedding <- umap(t(projection70_normalized$projection * (1-projection70_normalized$pval)), random_state = 100L)
...
```{r projection_normalized_expressed}
mat <- exprs(dat.filtered.expressed.normalized[expressed_genes,])
rownames(mat) <- gsub("\\..*", "", rownames(mat))
mat <- log10(mat+1)

projection70_normalized_expressed <- projectR(mat, linnarsson70, AnnotionObj = NA, IDcol=NA,full=TRUE)

tmp <- projection70_normalized_expressed$projection
rownames(tmp) <- paste0("Linnarsson_", rownames(tmp))
...
projection_embedding <- umap(t(projection70_normalized_expressed$projection), random_state = 100L)
...
projection_embedding <- umap(t(projection70_normalized_expressed$projection * (1-projection70_normalized_expressed$pval)), random_state = 100L)
...
```{r RF_model_class}
model <- readRDS("../Linnarsson_enteric_data/RFmodel.rds")

test_dat <- t(projection70$projection)
test_dat <- apply(test_dat, 2, function(x){ (x-mean(x))/sd(x)})
tmp <- dat.filtered
pData(tmp)$pred <- predict(model, newdata = test_dat)

###############

subset_model <- readRDS("../Linnarsson_enteric_data/RFmodel_subset.rds")

###############
test_dat <- t(projection70$projection)
test_dat <- apply(test_dat, 2, function(x){ (x-mean(x))/sd(x)})

pData(tmp)$subset_pred <- predict(subset_model, newdata = test_dat)
pData(tmp)$subset_pred_description <- NA
pData(tmp)[pData(tmp)$subset_pred %in% c("ENT4", "ENT5", "ENT6", "ENT9"), "subset_pred_description"] <- "Cholinergic enteric neurons"
pData(tmp)[pData(tmp)$subset_pred %in% c("ENT1", "ENT2", "ENT3"), "subset_pred_description"] <- "Nitrergic enteric neurons"
pData(tmp)[pData(tmp)$subset_pred %in% c("ENT7", "ENT8"), "subset_pred_description"] <- "Cholinergic enteric neurons, VGLUT2"
pData(tmp)[pData(tmp)$subset_pred %in% c("ENTG1"), "subset_pred_description"] <- "Enteric glia, proliferating"
pData(tmp)[is.na(pData(tmp)$subset_pred_description), "subset_pred_description"] <- "Enteric glia"

celltypeprobs <- predict(subset_model, newdata = test_dat, type = "prob")
melted_ctp <- melt(celltypeprobs)
max_prob <- as.data.frame(apply(celltypeprobs, 1, max))
names(max_prob) <- "prob"
mean(max_prob$prob)

pdf("classification1.pdf", width = 10)
plotUMAP(tmp, color = "subset_pred") + 
  theme(aspect.ratio = 1)
plotUMAP(tmp, color = "subset_pred_description") + 
  theme(aspect.ratio = 1)
plotUMAP(tmp, color = max_prob$prob) + 
  scale_color_viridis("Max probability") + 
  theme(aspect.ratio = 1)
ggplot(melted_ctp) + 
  geom_density(aes(x = value, color = variable))
ggplot(max_prob) + 
  geom_density(aes(x = prob)) +
  geom_vline(xintercept = mean(max_prob$prob), lty = 2)jk
ggplot(pData(tmp), aes(x = subset_pred_description)) + 
  geom_bar(aes(fill = subset_pred)) + 
  scale_fill_manual("Cluster\nClassification", values = colorRampPalette(brewer.pal(9, "Set1"))(length(unique(pData(tmp)$subset_pred)))) +
  geom_text(stat = "count", aes(label = ..count..), vjust = -0.5) +
  scale_y_continuous(expand = expand_scale(mult = c(0, .05))) +
  scale_x_discrete("Description Classification", labels = c("Cholinergic\nenteric\nneurons", "Cholinergic\nenteric\nneurons,\nVGLUT2", "Enteric glia", "Nitrergic\nenteric\nneurons"))
dev.off()

###############
test_dat <- t(projection70$projection * (1-projection70$pval))
test_dat <- apply(test_dat, 2, function(x){ (x-mean(x))/sd(x)})
...
pdf("classification2.pdf", width = 10)
plotUMAP(tmp, color = "subset_pred") + 
  theme(aspect.ratio = 1)
plotUMAP(tmp, color = "subset_pred_description") + 
  theme(aspect.ratio = 1)
...
###############
test_dat <- t(projection70_normalized$projection)
test_dat <- apply(test_dat, 2, function(x){ (x-mean(x))/sd(x)})
...
pdf("classification3.pdf", width = 10)
plotUMAP(tmp, color = "subset_pred") + 
  theme(aspect.ratio = 1)
plotUMAP(tmp, color = "subset_pred_description") + 
  theme(aspect.ratio = 1)
...
###############
test_dat <- t(projection70_normalized$projection * (1-projection70_normalized$pval))
test_dat <- apply(test_dat, 2, function(x){ (x-mean(x))/sd(x)})
...
pdf("classification4.pdf", width = 10)
plotUMAP(tmp, color = "subset_pred") + 
  theme(aspect.ratio = 1)
plotUMAP(tmp, color = "subset_pred_description") + 
  theme(aspect.ratio = 1)
...
###############
test_dat <- t(projection70_normalized_expressed$projection)
test_dat <- apply(test_dat, 2, function(x){ (x-mean(x))/sd(x)})
...
pdf("classification5.pdf", width = 10)
plotUMAP(tmp, color = "subset_pred") + 
  theme(aspect.ratio = 1)
plotUMAP(tmp, color = "subset_pred_description") + 
  theme(aspect.ratio = 1)
...
###############
test_dat <- t(projection70_normalized_expressed$projection * (1-projection70_normalized_expressed$pval))
test_dat <- apply(test_dat, 2, function(x){ (x-mean(x))/sd(x)})
...
pdf("classification6.pdf", width = 10)
plotUMAP(tmp, color = "subset_pred") + 
  theme(aspect.ratio = 1)
plotUMAP(tmp, color = "subset_pred_description") + 
  theme(aspect.ratio = 1)
...
plotUMAP(dat.filtered, color = "cluster") +
  scale_color_manual("Cluster", values = colorRampPalette(brewer.pal(9, "Set1"))(11), guide = guide_legend(ncol = 2)) +
  theme(aspect.ratio = 1, legend.position = c(0.8, 0.2))
