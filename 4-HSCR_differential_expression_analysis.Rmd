...
## y ~ genotype
```{r genotype}
genotype_diff_test <- differentialGeneTest(dat.filtered[expressed_genes,], fullModelFormulaStr = "~genotype + sex + batch + total_mRNAs", reducedModelFormulaStr = "~sex + batch + total_mRNAs", verbose = T)
...
#tmp <- t(scale(t(exprs(dat.filtered[genotype_diff_test_sig_genes,])), scale = T, center = T))
...
heatmap_data <- exprs(dat.filtered)[genotype_diff_test_sig_genes,]
heatmap_data <- t(apply(heatmap_data, 1, function(x){ # Normalize gene expr by dividing by SD
  x/sd(x)
}))
heatmap_data <- log10(heatmap_data + 1) # log transform
...
heatmap_annotation <- pData(dat.filtered)[, c("parentCelltype", "genotype")]
colnames(heatmap_annotation) <- c("Cell Type", "Genotype")

heatmap_colors <- list(
  "Cell Type" =  c("neuron" = "#56B4E9",
                   "progenitor/glia" = "#E69F00"),
  "Genotype" = c("het" = "#E41A1C", "hom" = "#377EB8"))

pdf("heatmap_DEG_genotype_2.pdf", height = 20, width = 20)
...
```{r genotype:celltype}
genotypeCellType_diff_test <- differentialGeneTest(dat.filtered[expressed_genes,], fullModelFormulaStr = "~genotype*celltype + sex + batch + total_mRNAs", reducedModelFormulaStr = "~genotype + celltype + sex + batch + total_mRNAs", cores = 1, verbose = T)
